...
async def cmd_set_welcome(message: types.Message):
    ...
    # Получаем текст после команды /set_welcome
    welcome_text = message.text.split(maxsplit=1)
    if len(welcome_text) < 2:
        return await message.reply("Неверный формат. Используйте: /set_welcome <текст приветствия>")

    await update_chat_setting(message.chat.id, 'welcome_message', welcome_text[1])
...
async def process_warning(message: types.Message, user_to_warn: types.User):
    ...
    if warnings_count >= warn_limit:
        # Возвращаем логику бана на 1 день
        try:
            await bot.ban_chat_member(chat_id, user_id, until_date=timedelta(days=1))
            await message.answer(
                f"🚫 Пользователь {hbold(user_to_warn.full_name)} получил {warnings_count} предупреждение и забанен на 1 день.",
                parse_mode="HTML"
            )
        except Exception as e:
            logging.error(f"Не удалось забанить пользователя {user_id} в чате {chat_id}: {e}")
            await message.answer(f"Не удалось забанить пользователя {hbold(user_to_warn.full_name)}. Возможно, у меня недостаточно прав.", parse_mode="HTML")
    else:
        await message.answer(
            f"⚠️ Пользователю {hbold(user_to_warn.full_name)} вынесено предупреждение ({warnings_count}/{warn_limit}).",
            parse_mode="HTML"
        )
...
async def cmd_set_warn_limit(message: types.Message):
    ...
    try:
        ...
        await message.answer(f"✅ Лимит предупреждений изменен на {hbold(limit)}.")
    except (IndexError, ValueError):
        await message.answer("Неверный формат. Используйте: /set_warn_limit <число>")
...
async def cmd_unwarn(message: types.Message):
    ...
    if await remove_last_warning(user_to_unwarn.id, message.chat.id):
        ...
        await message.answer(f"✅ Последнее предупреждение для {hbold(user_to_unwarn.full_name)} снято. Текущее количество: {warnings_count}.", parse_mode="HTML")
    else:
        await message.answer(f"У пользователя {hbold(user_to_unwarn.full_name)} нет предупреждений.", parse_mode="HTML")
...
async def cmd_clearwarns(message: types.Message):
    ...
    await message.answer(f"✅ Все предупреждения для пользователя {hbold(target_user.full_name)} были очищены.", parse_mode="HTML")
...
async def cmd_mute(message: types.Message):
    ...
    try:
        ...
        time_str = args[1] if len(args) > 1 else "1h" # по умолчанию 1 час
        duration = parse_time(time_str)
        if not duration:
            return await message.reply("Неверный формат времени. Используйте d, h, m (например, 1d, 2h, 30m).")
        ...
        await message.answer(f"🔇 Пользователь {hbold(user_to_mute.full_name)} замучен на {time_str}.", parse_mode="HTML")
        await message.delete()
    except Exception as e:
...
async def cmd_unmute(message: types.Message):
    ...
    # Возвращаем стандартные права (может отправлять все типы сообщений)
    await bot.restrict_chat_member(
        chat_id=message.chat.id,
    ...
    await message.answer(f"🔊 Пользователь {hbold(user_to_unmute.full_name)} размучен.", parse_mode="HTML")
    await message.delete()
...
async def cmd_ban(message: types.Message):
    ...
    if not message.reply_to_message:
        return await message.reply("Эта команда должна быть ответом на сообщение пользователя, которого вы хотите забанить.")
    ...
    try:
        ...
        # /ban 1d причина
        time_str = args[1] if len(args) > 1 else "1d" # по умолчанию 1 день
        ...
        await message.answer(
            f"🚫 Пользователь {hbold(user_to_ban.full_name)} забанен.\n"
            f"Срок: {time_str}\n"
            f"Причина: {reason}",
            parse_mode="HTML"
        )
        # Удаляем и команду, и сообщение, на которое ответили
        await message.delete()
...
async def cmd_unban(message: types.Message):
    ...
    if not message.reply_to_message:
        return await message.reply("Эта команда должна быть ответом на сообщение пользователя, которого вы хотите разбанить.")

    try:
        user_to_unban = message.reply_to_message.from_user
        # Для разбана нам нужен только chat_id и user_id
        await bot.unban_chat_member(chat_id=message.chat.id, user_id=user_to_unban.id)
        await message.answer(
            f"✅ Пользователь {hbold(user_to_unban.full_name)} успешно разбанен.",
            parse_mode="HTML"
        )
        await message.delete()
        ...
        await message.reply("Не удалось разбанить пользователя. Возможно, он и не был забанен в этом чате.")
...
async def cmd_add_word(message: types.Message):
    ...
    try:
        ...
        if await add_stop_word(message.chat.id, word):
            ...
            await message.answer(f"✅ Слово {hbold(word)} добавлено в черный список.")
        else:
            await message.answer("Это слово уже есть в списке.")
    except IndexError:
        await message.answer("Неверный формат. Используйте: /add_word <слово>")
...
async def cmd_del_word(message: types.Message):
    ...
    try:
        ...
        if await delete_stop_word(message.chat.id, word):
            ...
            await message.answer(f"✅ Слово {hbold(word)} удалено из черного списка.")
        else:
            await message.answer("Такого слова нет в списке.")
    except IndexError:
        await message.answer("Неверный формат. Используйте: /del_word <слово>")
...
async def cmd_antilink(message: types.Message):
    ...
    try:
        ...
        await message.answer("Неверный формат. Используйте: /antilink on или /antilink off")
...
async def message_filter(message: types.Message):
    ...
    if settings.get('antilink_enabled', False):
        if not await is_user_admin_silent(message.chat, message.from_user.id):
    ...
    for word in stop_words_cache.get(chat_id, set()):
        if word in text_lower:
            try:
                # Просто удаляем сообщение, без выдачи варна
                await message.delete()
            except Exception as e:
